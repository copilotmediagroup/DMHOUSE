...
Deno.serve(async(req)=>{if(req.method==='OPTIONS')return new Response('ok',{headers:cors});try{
 ...
 const r=await fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${resend}`,'Content-Type':'application/json'},body:JSON.stringify({from:'Data Market House <sales@debtpaper.com>',to:[data.email],subject:subject||'Your Data Market House Buyer Portal Access',html:body})});const out=await r.json();
