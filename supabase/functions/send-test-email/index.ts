...
Deno.serve(async(req)=>{if(req.method==='OPTIONS')return new Response('ok',{headers:cors});try{
 ...
 const response=await fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},body:JSON.stringify({from:`${Deno.env.get('OUTREACH_FROM_NAME')||'Data Market House'} <${fromEmail}>`,to:[recipient],reply_to:'sales@debtpaper.com',subject,html:String(body).replace(/\n/g,'<br>')})});
