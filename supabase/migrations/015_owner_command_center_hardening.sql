-- DMH Sales OS v1.1.1 — Owner Command Center hardening
-- Corrects pending-balance semantics and adds exact reservation deep links.
...
begin
  ...
  select jsonb_build_object(
    ...
    'pipeline', (select coalesce(jsonb_agg(x order by x.sort_order),'[]'::jsonb) from (
      ...
      union all select 5,'Reserved',count(*)::int,'/closings?tab=reservations' from reservations where company_id=v_company and status='active'
      union all select 6,'Sold',count(*)::int,'/closings?tab=sales' from sales where company_id=v_company
    ) x),
    'queues', jsonb_build_object(
      'pendingBalances', (select coalesce(jsonb_agg(jsonb_build_object('id',r.id,'title',p.name,'subtitle',a.name,'amount',greatest(r.amount-r.deposit_received-r.balance_received,0),'dueAt',r.payment_deadline,'path','/closings?tab=reservations&reservation='||r.id) order by r.payment_deadline),'[]'::jsonb) from reservations r join portfolios p on p.id=r.portfolio_id join agencies a on a.id=r.buyer_agency_id where r.company_id=v_company and r.status='active' and r.amount>r.deposit_received+r.balance_received),
      'expiringReservations', (select coalesce(jsonb_agg(jsonb_build_object('id',r.id,'title',p.name,'subtitle',a.name,'amount',r.amount,'dueAt',r.reservation_expires_at,'path','/closings?tab=reservations&reservation='||r.id) order by r.reservation_expires_at),'[]'::jsonb) from reservations r join portfolios p on p.id=r.portfolio_id join agencies a on a.id=r.buyer_agency_id where r.company_id=v_company and r.status='active' and r.reservation_expires_at between v_now and v_now+interval '48 hours'),
      'overdueDeposits', (select coalesce(jsonb_agg(jsonb_build_object('id',r.id,'title',p.name,'subtitle',a.name,'amount',greatest(r.deposit_required-r.deposit_received,0),'dueAt',r.payment_deadline,'path','/closings?tab=reservations&reservation='||r.id) order by r.payment_deadline),'[]'::jsonb) from reservations r join portfolios p on p.id=r.portfolio_id join agencies a on a.id=r.buyer_agency_id where r.company_id=v_company and r.status='active' and r.deposit_required>r.deposit_received and r.payment_deadline<v_now),
      ...
      'overdueFollowUps', (select coalesce(jsonb_agg(jsonb_build_object('id',f.id,'title',a.name,'subtitle',f.reason,'dueAt',f.due_at,'path','/agencies/'||a.id) order by f.due_at),'[]'::jsonb) from follow_ups f join agencies a on a.id=f.agency_id where f.company_id=v_company and f.completed_at is null and f.due_at<v_now),
      ...
      'pendingCommissions', (select coalesce(jsonb_agg(jsonb_build_object('id',c.id,'title',pr.full_name,'subtitle',p.name,'amount',c.amount,'dueAt',c.created_at,'path','/closings?tab=commissions') order by c.created_at),'[]'::jsonb) from commissions c join sales s on s.id=c.sale_id join portfolios p on p.id=s.portfolio_id join profiles pr on pr.id=c.employee_id where c.company_id=v_company and c.status in ('pending','approved'))
    ),
    'employees', (select coalesce(jsonb_agg(jsonb_build_object('id',pr.id,'name',pr.full_name,'outreach',coalesce(o.c,0),'offers',coalesce(ofr.c,0),'followUps',coalesce(fu.c,0),'sales',coalesce(sa.c,0),'revenue',coalesce(sa.revenue,0),'commission',coalesce(cm.amount,0)) order by coalesce(sa.revenue,0) desc,pr.full_name),'[]'::jsonb) from profiles pr left join lateral (select count(*) c from outreach_activities x where x.employee_id=pr.id and x.company_id=v_company) o on true left join lateral (select count(*) c from offers x where x.employee_id=pr.id and x.company_id=v_company) ofr on true left join lateral (select count(*) c from follow_ups x where x.employee_id=pr.id and x.company_id=v_company and x.completed_at is not null) fu on true left join lateral (select count(*) c,coalesce(sum(sale_price),0) revenue from sales x where x.winning_employee_id=pr.id and x.company_id=v_company) sa on true left join lateral (select coalesce(sum(amount),0) amount from commissions x where x.employee_id=pr.id and x.company_id=v_company) cm on true where pr.company_id=v_company and pr.role='employee' and pr.is_active),
