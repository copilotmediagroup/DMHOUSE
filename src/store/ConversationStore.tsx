...
export type ConversationMessage={id:string;conversationId:string;agencyId:string;senderProfileId?:string;direction:'outbound'|'inbound'|'internal';fromEmail?:string;toEmail?:string;subject?:string;body:string;isRead:boolean;createdAt:string};
...
type Store={conversations:Conversation[];messages:ConversationMessage[];employees:EmployeeOption[];tests:any[];loading:boolean;error:string;refresh:()=>Promise<void>;ensure:(agencyId:string)=>Promise<string>;setState:(id:string,input:{status?:string;assignedEmployeeId?:string;ownerJoined?:boolean;takeOwnership?:boolean;releaseOwnership?:boolean})=>Promise<void>;setWorkflow:(id:string,input:WorkflowInput)=>Promise<void>;markRead:(conversationId:string)=>Promise<void>;sendReply:(conversationId:string,agencyId:string,recipient:string,subject:string,body:string)=>Promise<void>;addInternalNote:(conversationId:string,body:string)=>Promise<void>;sendTest:(recipient:string,subject:string,body:string)=>Promise<void>};
...
export function ConversationProvider({children}:{children:ReactNode}){
 ...
 setMessages((m.data||[]).map((r:any)=>({id:r.id,conversationId:r.conversation_id,agencyId:r.agency_id,senderProfileId:r.sender_profile_id||undefined,direction:r.direction,fromEmail:r.from_email||undefined,toEmail:r.to_email||undefined,subject:r.subject||undefined,body:r.body,isRead:!!r.is_read,createdAt:r.created_at})));
 ...
 const markRead=async(conversationId:string)=>{const {error:e}=await supabase.from('conversation_messages').update({is_read:true}).eq('conversation_id',conversationId).eq('direction','inbound');if(e)throw e;await refresh()};
 const sendReply=async(conversationId:string,agencyId:string,recipient:string,subject:string,body:string)=>{if(!profile)throw new Error('Profile unavailable.');const {data,error:e}=await supabase.rpc('dmh_queue_outreach_email',{p_agency_id:agencyId,p_contact_id:null,p_portfolio_id:null,p_template_id:null,p_recipient:recipient,p_subject:subject,p_body:body,p_follow_up_at:null});if(e)throw e;const {data:fn,error:fe}=await supabase.functions.invoke('send-outreach-email',{body:{messageId:data}});if(fe)throw fe;if(fn?.ok===false)throw new Error(fn.error);await supabase.from('conversations').update({last_message_at:new Date().toISOString(),last_outbound_at:new Date().toISOString(),subject,updated_at:new Date().toISOString()}).eq('id',conversationId);await refresh()};
 ...
 return <C.Provider value={useMemo(()=>({conversations,messages,employees,tests,loading,error,refresh,ensure,setState,setWorkflow,markRead,sendReply,addInternalNote,sendTest}),[conversations,messages,employees,tests,loading,error,refresh])}>{children}</C.Provider>
}
