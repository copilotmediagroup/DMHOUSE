...
type CreateInput = Omit<Portfolio,'id'|'createdAt'|'status'|'file'|'unmaskedFile'> & { file?: File; unmaskedFile?: File };
type Store={
 ...
 uploadMaskedFile:(id:string,file:File)=>Promise<void>;
 uploadUnmaskedFile:(id:string,file:File)=>Promise<void>;
...
function mapFile(file:any):PortfolioFile|undefined{return file?{id:file.id,name:file.file_name,size:Number(file.size_bytes||0),type:file.mime_type||'text/csv',uploadedAt:file.created_at,storagePath:file.storage_path}:undefined}
function mapPortfolio(row:any,file?:any,unmasked?:any):Portfolio{return {
 ...
 status:row.status,createdAt:row.created_at,activatedAt:row.activated_at||undefined,file:mapFile(file),unmaskedFile:mapFile(unmasked)
};}
...
export function PortfolioProvider({children}:{children:ReactNode}){
 ...
 const refresh=useCallback(async()=>{setLoading(true);setError('');try{
  ...
  let unmaskedByPortfolio=new Map<string,any>();
  if(profileRow.role==='owner'){
    const {data:unmasked,error:unmaskedError}=await supabase.from('portfolio_unmasked_files').select('*').eq('company_id',profileRow.company_id).is('superseded_at',null).order('created_at',{ascending:false});
    if(unmaskedError && unmaskedError.code!=='42P01')throw unmaskedError;
    unmaskedByPortfolio=new Map<string,any>();for(const f of unmasked||[])if(!unmaskedByPortfolio.has(f.portfolio_id))unmaskedByPortfolio.set(f.portfolio_id,f);
  }
  setPortfolios((rows||[]).map(r=>mapPortfolio(r,byPortfolio.get(r.id),unmaskedByPortfolio.get(r.id))));
 ...
 async function uploadMaskedForPortfolio(portfolioId:string,file:File){if(!profile)throw new Error('Owner profile is not ready.');const current=portfolios.find(p=>p.id===portfolioId)?.file;
  const path=`${profile.company_id}/${portfolioId}/${Date.now()}-${safeName(file.name)}`;const upload=await supabase.storage.from('portfolio-files').upload(path,file,{contentType:file.type||'text/csv',upsert:false});if(upload.error)throw upload.error;
  const {data:latest}=await supabase.from('portfolio_files').select('version').eq('portfolio_id',portfolioId).order('version',{ascending:false}).limit(1).maybeSingle();
  const meta=await supabase.from('portfolio_files').insert({company_id:profile.company_id,portfolio_id:portfolioId,storage_path:path,file_name:file.name,version:Number(latest?.version||0)+1,employee_visible:true,size_bytes:file.size,mime_type:file.type||'text/csv'});if(meta.error){await supabase.storage.from('portfolio-files').remove([path]);throw meta.error;}
  if(current?.storagePath){await supabase.from('portfolio_files').update({locked_at:new Date().toISOString()}).eq('id',current.id);}
 }
 async function uploadUnmaskedForPortfolio(portfolioId:string,file:File){if(!profile)throw new Error('Owner profile is not ready.');
  const path=`${profile.company_id}/${portfolioId}/${Date.now()}-${safeName(file.name)}`;const upload=await supabase.storage.from('portfolio-unmasked-vault').upload(path,file,{contentType:file.type||'application/octet-stream',upsert:false});if(upload.error)throw upload.error;
  const {error:registerError}=await supabase.rpc('dmh_register_portfolio_unmasked_file',{p_portfolio_id:portfolioId,p_file_name:file.name,p_storage_path:path,p_size_bytes:file.size,p_mime_type:file.type||'application/octet-stream'});
  if(registerError){await supabase.storage.from('portfolio-unmasked-vault').remove([path]);throw registerError;}
 }
 const createPortfolio=async(input:CreateInput)=>{if(!profile)throw new Error('Owner profile is not ready.');
  const {file,unmaskedFile,...values}=input; const {data:row,error:insertError}=await supabase.from('portfolios').insert({company_id:profile.company_id,created_by:profile.id,name:values.name,original_creditor:values.originalCreditor,category:values.category,account_count:values.accountCount,face_value:values.faceValue,asking_price:values.askingPrice,private_minimum:values.privateMinimum,acquisition_cost:values.acquisitionCost,employee_commission_type:values.employeeCommissionType,employee_commission_value:values.employeeCommissionValue,employee_commission_visible:values.employeeCommissionVisible,description:values.description,selling_points:values.sellingPoints,status:'draft'}).select('*').single();if(insertError)throw insertError;
  try{if(file)await uploadMaskedForPortfolio(row.id,file);if(unmaskedFile)await uploadUnmaskedForPortfolio(row.id,unmaskedFile);}catch(e){await supabase.from('portfolios').delete().eq('id',row.id);throw e;}
  await refresh();const created=(await supabase.from('portfolios').select('*').eq('id',row.id).single()).data;return mapPortfolio(created);
 };
 ...
 const uploadMaskedFile=async(id:string,file:File)=>{await uploadMaskedForPortfolio(id,file);await refresh()};
 const uploadUnmaskedFile=async(id:string,file:File)=>{await uploadUnmaskedForPortfolio(id,file);await refresh()};
 ...
 const removePortfolio=async(id:string)=>{const p=portfolios.find(x=>x.id===id);const check=await supabase.rpc('dmh_assert_portfolio_deletable',{p_portfolio_id:id});if(check.error)throw check.error;if(p?.file?.storagePath){const r=await supabase.storage.from('portfolio-files').remove([p.file.storagePath]);if(r.error)throw r.error;}if(p?.unmaskedFile?.storagePath){const r=await supabase.storage.from('portfolio-unmasked-vault').remove([p.unmaskedFile.storagePath]);if(r.error)throw r.error;}const {error:e}=await supabase.from('portfolios').delete().eq('id',id);if(e)throw e;setPortfolios(x=>x.filter(p=>p.id!==id));};
 ...
 const value=useMemo<Store>(()=>({portfolios,audit,role,profile,loading,error,setRole:setRoleState,refresh,createPortfolio,updatePortfolio,uploadMaskedFile,uploadUnmaskedFile,transition,removePortfolio,getDownloadUrl,active:portfolios.find(p=>['active','negotiating','reserved','payment_pending'].includes(p.status))}),[portfolios,audit,role,profile,loading,error,refresh]);
